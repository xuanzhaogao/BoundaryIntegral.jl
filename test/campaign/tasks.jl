using BoundaryIntegral, Test, LinearAlgebra
...
@testset "eval_batch_core propagates lattice_basis on a skewed grid" begin
    ...
    #
    # Promoted here (from test/campaign/pipeline.jl, which only runs under
    # BI_RUN_FULL_TESTS=1) so this guard actually executes in CI: measured
    # ~3 s in isolation, well under the default suite's per-file budget.
    nx = ny = nz = 4
    a1 = (1.0, 0.0, 0.0)
